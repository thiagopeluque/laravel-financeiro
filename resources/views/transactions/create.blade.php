<x-app-layout>
                    ...
                    <form method="POST" action="{{ route('transactions.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative mt-1 rounded-md shadow-sm">
                                ...
                                <input type="number" name="valor" id="valor" value="{{ old('valor') }}" required min="0.01" step="0.01" class="block w-full rounded-md border-gray-300 pl-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="0,00">
                            </div>
                            @error('valor')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="descricao" id="descricao" value="{{ old('descricao') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Ex: Salário mensal">
                            @error('descricao')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="data" id="data" value="{{ old('data', date('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error('data')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="observacoes" id="observacoes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Informações adicionais...">{{ old('observacoes') }}</textarea>
                            @error('observacoes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4 border-t pt-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="recorrente" id="recorrente" value="1" {{ old('recorrente') ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        ...
                        <div id="recorrente-options" class="mb-4 {{ old('recorrente') ? '' : 'hidden' }}">
                            <label for="recorrente_ate" class="block text-sm font-medium text-gray-700">Até qual Mês / Ano</label>
                            <input type="month" name="recorrente_ate" id="recorrente_ate" value="{{ old('recorrente_ate') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
    ...
    <script>
        document.getElementById('recorrente').addEventListener('change', function() {
            const optionsDiv = document.getElementById('recorrente-options');
            if (this.checked) {
